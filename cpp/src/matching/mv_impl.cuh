...
void bfs(raft::handle_t const& handle,
         ...
         typename GraphViewType::vertex_type* mate,
         size_t n_sources,
  ...
  while (true) {
    if (direction_optimizing) {
      ...
      update_v_frontier(
        ...
        [vertex_partition,mate,depth] __device__(auto v, auto v_val, auto pushed_val) {
          auto match = *(mate + vertex_partition.local_vertex_partition_offset_from_vertex_nocheck(v));
          auto update = (v_val == invalid_distance) && (depth%2)==(match==pushed_val);
